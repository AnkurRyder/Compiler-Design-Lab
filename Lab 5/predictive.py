		...
		while top != '$':
			...
			if top == seq[si]:
				...
				try:
					...
					stack.pop()
					if prod == [""]:
						print("derive {0} on `{1}` to: ε".format(top, seq[si]))
					else:
						print("derive {0} on `{1}` to: {2}".format(top, seq[si], " ".join(prod)))
